...
Outils exposes :
  ...
  - envoyer_email      : ENVOI REEL (action "send") -- version Thai, routine autonome.
                         KILL-SWITCH : si un fichier PAUSE existe a la racine du repo,
                         l'envoi est REFUSE par le serveur (un brouillon est cree a la place).
...
def _pause_active() -> bool:
    """Kill-switch : un fichier PAUSE a la racine du repo coupe tout envoi reel.

    Verifie le cwd du serveur (lance depuis la racine via .mcp.json) ET le parent
    de tools/ (au cas ou le cwd differe). Belt-and-suspenders avec la regle du prompt.
    """
    candidates = [
        os.path.join(os.getcwd(), "PAUSE"),
        os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "PAUSE"),
    ]
    return any(os.path.exists(p) for p in candidates)


def tool_envoyer_email(args: dict) -> str:
    """ENVOI REEL. Memes regles de rendu que creer_brouillon (HTML + signature auto)."""
    to = (args.get("to") or "").strip()
    objet = (args.get("objet") or args.get("subject") or "").strip()
    corps = args.get("corps") or args.get("body") or ""
    if not to or not objet or not corps.strip():
    ...
    if isinstance(cc, list) and cc:
        body["cc"] = [{"email": str(a), "name": ""} for a in cc]
    if _pause_active():
        create_draft(_address(), body)
        return (
            f"ENVOI REFUSE : kill-switch PAUSE actif (fichier PAUSE a la racine du repo). "
            f"Un BROUILLON a ete cree a la place dans {_address()} pour {to}. "
            "Supprime le fichier PAUSE pour reactiver l'envoi."
        )
    send_email(_address(), body)
    return f"EMAIL ENVOYE depuis {_address()} a {to} (objet : {objet})."
...
TOOLS = {
    ...
    "envoyer_email": {
        "fn": tool_envoyer_email,
        "description": (
            "ENVOIE REELLEMENT un email depuis la boite Infomaniak (pas un brouillon : le "
            "prospect le recoit immediatement). Reserve a la routine Thai autonome, UNIQUEMENT "
            "apres la checklist pre-envoi de CLAUDE.md (T1 + email source fiable + faits "
            "verifies). Corps en HTML (paragraphes propres + signature auto), comme "
            "creer_brouillon. KILL-SWITCH : si un fichier PAUSE existe a la racine du repo, "
            "l'envoi est refuse et un brouillon est cree a la place."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
                "to": {"type": "string", "description": "adresse du destinataire (le prospect)"},
                "objet": {"type": "string", "description": "objet du mail"},
                "corps": {"type": "string", "description": "corps du mail (texte avec accents ; un paragraphe par ligne vide). NE PAS inclure la signature : elle est ajoutee automatiquement."},
                "cc": {"type": "array", "items": {"type": "string"}, "description": "copies (optionnel)"},
                "html": {"type": "boolean", "description": "true/omis = HTML formate (defaut). false = texte brut sans signature."},
                "signature": {"type": "boolean", "description": "ajouter la signature en HTML (defaut: true ; ignore si html=false)"},
            },
            "required": ["to", "objet", "corps"],
        },
    },
